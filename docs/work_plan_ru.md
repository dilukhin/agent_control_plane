...
**R3.3 реализован:** явное завершение задач, 30 суток хранения конечной истории, семидневное окно сообщений и сохраняемая граница повторной доставки; очистка ограниченными пакетами и incremental vacuum. Активные/неопределённые операции и их доказательства защищены. Схема v3; правила, ограничения и продолжение пакетов: [retention.md](retention.md). Приёмка точных PR/main commits и ссылки на CI фиксируются в #10.

**R3.1–R3.3 выполнены. Следующая задача — #11 / R4:** контракты worker/provider и две заменяемые тестовые реализации без изменения orchestration. Затем #12 / R5 с выбором первого транспорта; после них реализация #8 / R6. Эти этапы ещё не реализованы. Перед началом проверить текущий main и issue.
